...
public class ShowFinishedGamesServlet extends HttpServlet {
       ...
       String games = "";
        ...
        Filter eq_filter =
        		  new FilterPredicate("SenderId",
        		                      ...
        		                      user_id);
        Query q = new Query("FinishedGames").setFilter(eq_filter);
        PreparedQuery pq = datastore.prepare(q);        
        for (Entity result1 : pq.asIterable()) {
        	String game = result1.getProperty("GameId")+"";
        	if (games.toLowerCase().contains(game.toLowerCase())==false){
        		games = games + game + ",";
        	}	
        }
        if (games.length()>0) {
